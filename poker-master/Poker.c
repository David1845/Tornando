...
// creates the Key values for the json dictionary with a max of 16 data points.
void getKeys(char keys[][16]){
    memset(keys, 0, sizeof(keys[0][0]) * 16* 16);
    strncpy(keys[0],"High Card",strlen("High Card"));
    strncpy(keys[1],"Pair",strlen("Pair"));
    strncpy(keys[2],"Two Pair",strlen("Two Pair"));
    strncpy(keys[3],"Tripes",strlen("Tripes"));
    strncpy(keys[4],"Straight",strlen("Straight"));
    strncpy(keys[5],"Flush",strlen("Flush"));
    strncpy(keys[6],"Full House",strlen("Full House"));
    strncpy(keys[7],"Quads",strlen("Quads"));
...
void SetHands(struct hands *AllHands){
    ...
    strcpy(AllHands[0].hand[0],"A\u2660");
    strcpy(AllHands[0].hand[1],"K\u2666");
    
    strcpy(AllHands[1].hand[0],"A\u2666");
    strcpy(AllHands[1].hand[1],"Q\u2666");
...
int BestFullHouse(pokerutil * util, struct hands * AllHands, int *pos){
    ...
    /// if hand 0 contains a fullhouse
    ...
    for(int i =0;i<1;i++){
        ...
        if(BestQuads(util, AllHands, &Quads)){
           ...
           if(BestFH[1] > -1){
            ...
            AllHands[BestHighcard].highCard++;
...
void setOfAllFullHouses(hands Hand){
    // hand = A K
    // I want to generate only fullhouse hands
    int fullhouseList[1000][7];
    int tempList[7];
  
    for(int i = 0; i < 5 ;i++){
        if(i < 2){
            
        tempList[i] = Hand->results[0]->Strength;
            
        }else if(i >= 2){
            
            tempList[i] = Hand->results[1]->Strength;
        }
    ...
    for(int j = 2; j < 14; j++){
        ...
        for(int i = 2; i < 14; i++){
            tempList[5] = i;
            memcpy(fullhouseList[i],tempList,sizeof(tempList));
            i++;
        }
...
struct hands * runPokerSimulation(){
    ...
    for(int i=0;i<4;i++){
        staticFlop(&util, AllHands,pairTestData[i]);
        
        startSimulation(&util,AllHands);
       
    }
    
   // setOfAllFullHouses(&AllHands[0]);
    return AllHands;
}
