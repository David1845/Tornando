...
void staticFlop(pokerutil * util, struct hands * AllHands){

    util->CustomSet[0][0] = 'K';
    util->CustomSet[1][0] = 'A';
    util->CustomSet[2][0] ='J';
    
    for(int j = 0;j<3;j++){
        for(int i = NumPlayers*2; i < 52; i++){
...
void SetHands(struct hands *AllHands){
    ...
    AllHands[0].hand[0][0]='K';
    AllHands[0].hand[1][0]='J';
    
    AllHands[1].hand[0][0]='Q';
    AllHands[1].hand[1][0]='A';
...
int bestTwoPair(struct hands * AllHands,pokerutil * util, int * currentBestTwoPair){
        ...
        for(int i =0; i<Handset;i++){
            if(AllHands[i].besthand.twopair == 1 && currentBestTwoPair >= 0 ){
                *currentBestTwoPair = i;
        }else if(AllHands[i].hand[0][0] >= AllHands[*currentBestTwoPair].hand[0][0] && AllHands[i].besthand.twopair == 1 ){
            *currentBestTwoPair = i;
        }
    }
    if(AllHands[*currentBestTwoPair].besthand.twopair==1 ){
...
void StatsOfAllIterations(struct hands * AllHands){
    ...
    for(int i=0 ;i<Handset;i++){
        ...
        printf("\n");
        printf("odds of getting pair on the flop %f\n",(float)AllHands[i].pair/1);
        printf("odds of getting 2 pair on the flop %f\n",(float)AllHands[i].twopair/1);
        printf("odds of getting tripes on the flop %f\n",(float)AllHands[i].tripes/1);
    }
}

void startSimulation(pokerutil * util, struct hands * AllHands){
    int currentBestTwoPair;
    int currentBestPair;
    for(int i =0 ;i<1;i++){
        currentBestPair =0;
        currentBestTwoPair = -1;
        for(int j=0; j<Handset;j++){
            handAsAnyPairOrFullHouse(j,AllHands,util);
        }
    
        if(bestTwoPair(AllHands,util,&currentBestTwoPair) == 1){
      
            AllHands[currentBestTwoPair].wonHand++;
            
        }else if(BestPair(AllHands,util,&currentBestPair) == 1){
           // displayFlop(util);
           // printf("%s,%s\n",AllHands[currentBestPair].hand[0],AllHands[currentBestPair].hand[1]);
            AllHands[currentBestPair].wonHand++;
        }
...
void clearhands(struct hands * AllHands){
